...
@mock.patch.dict(os.environ, {f"EXAMPLE_SECRET": '{"key": "value"}'})
...
@mock.patch.dict(os.environ, {f"EXAMPLE_SECRET": '"example_value"'})
...
def test_fetch_aws_secret():
    from datasets.utils.secret import secret_cache, logger
    ...
    # Test force_reload
    assert Secret(aws_secret_arn="json-decodable-dict", 
        key="key", force_reload=True).value == "new_value"
